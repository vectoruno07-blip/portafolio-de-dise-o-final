  ...
  return (
    <header className={`sticky top-0 z-40 backdrop-blur-xl border-b transition-colors duration-200 ${
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-2.5 flex items-center justify-between gap-2">
        {/* Brand & Personal Logo */}
        <div 
          className="flex items-center gap-2.5 sm:gap-3 min-w-0 cursor-pointer shrink-0"
          onClick={() => {
            if (typeof window !== 'undefined') {
              const count = Number(sessionStorage.getItem('logo_clicks') || '0') + 1;
              sessionStorage.setItem('logo_clicks', String(count));
              if (count >= 5) {
                localStorage.setItem('admin_mode', 'true');
                sessionStorage.removeItem('logo_clicks');
                alert('¡Modo administrador activado en este navegador!');
                window.location.reload();
              }
            }
          }}
          title="Víctor Hugo González - Portafolio Profesional"
        >
